...
class UnoRoom {
  ...
  addPlayer(socketId, name, isBot = false) {
    ...
    const player = {
      socketId: isBot ? `bot-${Date.now()}-${Math.random().toString(36).slice(2,8)}` : socketId,
  ...
  startTurnTimer() {
    ...
    if (current.isBot) {
      ...
      this.turnTimer = setTimeout(() => this.handleTimeout(), this.turnTimeLimit);
    }
  }
  ...
  canPlay(card, topCard, wildColor, drawStack) {
    ...
    if (drawStack > 0 && !this.allowStacking) return false;
  ...
  applyCardEffects(card) {
    switch (card.type) {
      case CARD_TYPES.SKIP: this.advanceTurn(); break;
      case CARD_TYPES.REVERSE:
        this.game.direction *= -1;
        if (this.players.length === 2) this.advanceTurn();
        break;
      case CARD_TYPES.DRAW_TWO: this.game.drawStack += 2; break;
      case CARD_TYPES.WILD_DRAW_FOUR: this.game.drawStack += 4; break;
    }
  }
  ...
  drawCard(socketId) {
    ...
    this.broadcast('playerDrewCards', { socketId: player.socketId, count: drawn.length, reason: 'normal' });
    this.broadcast('gameUpdate', this.getPublicState());
    return { success: true, drawn };
  }
